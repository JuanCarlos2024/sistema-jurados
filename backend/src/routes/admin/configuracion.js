...
router.post('/administradores', async (req, res) => {
    ...
    const ROLES_EVAL = ['jefe_area', 'analista', 'comision_tecnica', 'monitor', 'director', 'capacitador'];
    const rolEval = rol_evaluacion || null;
    if (rolEval && !ROLES_EVAL.includes(rolEval)) {
        return res.status(400).json({ error: 'rol_evaluacion debe ser jefe_area, analista, comision_tecnica, monitor, director o capacitador' });
    }
